...
"""
SDWFCM3.py : Spatial Distance Weighted Fuzzy C-Means clustering algorithm in 3d.
...
Functions:
    neighbours3 - Return list of neighbouring pixels.
    sdfcm_cmeans3 - Spatial Distance Weighted Fuzzy C-Means clustering algorithm in 3d.
"""
...
def neighbours3(im, z, x, y, d=1):
    """Return list of neighbouring pixels.

    :param im: image
    :param z: stacked coordinate
    ...
    """
    zz, w, h = tuple(im.shape)

    z_min = z - d if z > d - 1 else z
    x_min = x - d if x > d - 1 else x
    y_min = y - d if y > d - 1 else y
    z_max = z + d if z < zz - d else z
    x_max = x + d if x < w - d else x
    y_max = y + d if y < h - d else y

    n = []
    nlist = list(itertools.product(range(z_min, z_max + 1),
                                   range(x_min, x_max + 1),
                                   range(y_min, y_max + 1)))
    nlist.remove((z, x, y))

    for (iz, ix, iy) in nlist:
        n.append(im[iz, ix, iy])

    return (n, nlist)


def _sdfcm_cmeans03(data, u_old, c, m, z, w, h,
                    l, number_neighbours):
    """
    ...
    d_reshaped = np.ndarray.reshape(d, (c, z, w, h))

    f = np.zeros((c, z, w, h))

    for (iz, ix, iy) in itertools.product(range(0, z), range(0, w), range(0, h)):

        d_neighbours_sum_by_clust = np.zeros((c, 1))

        for i in range(c):
            d_neighbours = neighbours3(d_reshaped[i, :, :], iz, ix, iy, d=number_neighbours)[0]
            d_neighb_sum = np.sum(d_neighbours)
            d_neighbours_sum_by_clust[i] = d_neighb_sum
            f[i, iz, ix, iy] = d_neighb_sum

        d_neighbours_sum_min = np.min(d_neighbours_sum_by_clust)

        f[:, iz, ix, iy] /= d_neighbours_sum_min

    f = np.reshape(f, (c, z * w * h))
...
def sdfcm_cmeans3(data, c, m, z, w, h, l, number_neighbours,
    ...
    while p < maxiter - 1:
        u2 = u.copy()
        [cntr, u, Jjm, d] = _sdfcm_cmeans03(data, u2, c, m, z, w, h,
                                            l, number_neighbours)
        jm = np.hstack((jm, Jjm))
        p += 1
